...
Este trabalho apresenta uma simulação original de \textit{show} de drones em ambiente de grade discreta, desenvolvida em linguagem Python sem dependências externas obrigatórias (\textit{pip}). Os \textbf{dois agentes autônomos} exigidos pelo trabalho são o \textbf{coreógrafo} (deliberação de alto nível com LLM via API Groq, \textit{prompt} e saída JSON) e o \textbf{piloto} (agente \textbf{baseado em modelo}, com BFS e anti-colisão). Os quatro drones são \textbf{entidades executoras} no ambiente, não agentes autônomos adicionais. O ambiente inclui múltiplos obstáculos, resolução de alvos inviáveis e modo de demonstração quando a API não está disponível. O repositório público contém o código-fonte e instruções de execução. O documento formaliza a especificação PEAS e a caracterização do ambiente segundo as dimensões de Russell e Norvig.
...
Para evitar ambiguidade em relação ao enunciado, explicita-se que \textbf{os dois agentes autônomos exigidos pelo trabalho são}: (i) o \textbf{agente coreógrafo}, responsável pela deliberação de alto nível com apoio de LLM (escolha e parâmetros da formação); e (ii) o \textbf{agente piloto}, responsável pelo planejamento e pela execução segura dos deslocamentos no ambiente (um passo por \textit{beat} por drone, com BFS e regras de anti-colisão). Os \textbf{quatro drones} não são contados como agentes autônomos adicionais: são \textbf{entidades executoras} no ambiente, cujos movimentos resultam do plano único do piloto e das restrições espaciais compartilhadas.
...
O sistema é \textbf{multiagente} porque envolve \textbf{dois agentes autônomos} com funções complementares --- coreógrafo e piloto --- atuando sobre um \textbf{ambiente compartilhado}. Além disso, a execução envolve \textbf{múltiplos drones}, que introduzem restrições espaciais e necessidade de coordenação (precedência no deslocamento, não colisão). A interação é cooperativa no objetivo comum de formação, com competição apenas pelo uso seguro das células.
...
O ambiente é \textbf{dinâmico} porque o estado relevante \textbf{evolui ao longo dos \textit{beats}}, exigindo \textbf{replanejamento contínuo} dos agentes em função das novas posições dos drones (e das metas de formação quando o coreógrafo as atualiza).
...
\subsection{Fluxo da simulação}

Resumo do ciclo por \textit{beat} (útil para visão global antes dos detalhes de implementação):

\begin{itemize}[leftmargin=1.25cm]
  \item O coreógrafo observa o estado atual do ambiente.
  \item O LLM define a próxima formação em JSON.
  \item O ambiente traduz a formação em células-alvo.
  \item O piloto planeja os movimentos via BFS.
  \item Os drones executam um passo.
  \item O ciclo repete-se até convergência.
\end{itemize}
...
O \textbf{agente coreógrafo} implementa deliberação de alto nível com LLM: \textit{prompt} de sistema fixo exige resposta em JSON (\texttt{formation}, \texttt{center\_row}, \texttt{center\_col}, \texttt{scale}, \texttt{note}). O \textbf{agente piloto} mantém modelo interno da grade; para cada drone, calcula o passo com busca em largura (BFS) na grade, considerando obstáculos, posições alheias e intenções já planejadas de drones com índice menor, alinhando alvos por proximidade para reduzir bloqueios por permutação. A \textbf{busca em largura (BFS)} foi escolhida por sua adequação a \textbf{ambientes em grade discreta com custo uniforme por ação}, garantindo \textbf{caminhos mínimos em número de passos} até o alvo alcançável. Repete-se, no nível de implementação, que apenas esses dois módulos correspondem aos \textbf{dois agentes autônomos}; os drones materializam o efeito conjunto das ações escolhidas pelo piloto.

\section{CONCLUSÃO}

A simulação cumpre os requisitos do trabalho avaliativo: ambiente original em Python, \textbf{dois agentes autônomos} (coreógrafo com LLM e piloto baseado em modelo), integração com engenharia de \textit{prompt} e planejamento geométrico com BFS. A documentação no repositório e o presente relatório atendem à especificação PEAS, à classificação dimensional e às instruções de reprodutibilidade.

A \textbf{principal contribuição} do projeto está na \textbf{separação entre deliberação de alto nível baseada em LLM} e \textbf{execução segura via planejamento determinístico} (BFS e regras de precedência na grade), o que torna a arquitetura híbrida auditável e alinhada à literatura clássica. Como \textbf{limitação}, a escolha das formações depende da \textbf{variabilidade do modelo} (e, no modo demonstração, de regras fixas), e \textbf{não há avaliação quantitativa de eficiência global} do espetáculo; o emparelhamento drone--alvo por proximidade permanece heurístico. Como extensões futuras, caberia modelar incerteza parcial ou custos de bateria, e medir formalmente tempo até convergência ou otimalidade em cenários parametrizados.
